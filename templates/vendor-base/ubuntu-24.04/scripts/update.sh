...
echo "purge needrestart to prevent sshd restart that breaks packer SSH channel"
apt-get -y purge needrestart 2>/dev/null || true;

echo "upgrade all installed packages incl. kernel and kernel headers"
NEEDRESTART_MODE=a apt-get -y dist-upgrade -o Dpkg::Options::="--force-confnew";
