...
#floating point.
#looks like qemu-img info has changed in ubuntu 24.04 and now returns more then one line so we take only the last value
IMG_SIZE=$(qemu-img info --output=json "$TMP_IMG" | awk '/virtual-size/{s=int($2)/(1024^3); print (s == int(s)) ? s : int(s)+1 }' | tail -1)
